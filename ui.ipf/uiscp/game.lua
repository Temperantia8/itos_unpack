...
function GET_FULL_NAME(item, useNewLine, isEquiped, timeImgSize)
	...
	-- ?§Î≤Ñ Î¶¨Ïù∏?¨Ïä§ ÎπÑÍ∏â???ÅÏö©?òÎ©¥ Í∞íÏù¥ ?åÏàòÎ°??§Ïñ¥?§Î?Î°??ëÏàòÎ°?Î∞îÍøîÏ§?
...
function GET_TOTAL_MONEY_STR() 
	-- intÎ°??òÎ¶¨ÏßÄ ?äÍ≥† ?∂ÏúºÎ©??¥Í±∏ ?∞ÎèÑÎ°ùÌïò?? Î∞ñÏóê??Í≥ÑÏÇ∞???åÎäî tonumber?òÍ±∞??BigNumber ?ÑÏö© ?®Ïàò???¨Ïö© Í∂åÏû•
	local silver = '0';
...
function USE_ITEMTARGET_ICON(frame, itemobj, argNum)
	...
	if itemobj.GroupName == "Gem" then
		...
		local propPenaltyAdd = socketPenaltyProp:GetPropPenaltyAddByIndex(0, 0); -- ?§ÌÇ¨ ?¨Ïù∏ÏßÄ Í≤Ä??
		if propPenaltyAdd ~= nil and itemobj.GemRoastingLv < itemobj.GemLevel then
			-- ???àÎ≤®??Î°úÏä§???àÎ≤®Î≥¥Îã§ ??? Í≤ΩÏö∞ Í≤ΩÍ≥†Ï∞?
...
function ON_RIDING_VEHICLE(onoff)
    ...
    --js: ?ÑÏû¨ Î©îÎ•¥Ïπ¥Î∞îÎß??òÎìúÏΩîÎî© ?ïÌÉúÎ°??àÏô∏Ï≤òÎ¶¨?òÏñ¥?àÎã§ ?ÑÏóê ?®ÏàòÎ™ÖÎßå Î¥êÎèÑ ?åÏàò?àÏùå, ?¥Îãπ ?àÏô∏Ï≤òÎ¶¨ Ï∂îÌõÑ ?òÍ∏∞Î°??ïÏàò?®Ï? ?¥ÏïºÍ∏∞Ìï® (2.0?ùÎÇòÍ≥?--
...
function KEYBOARD_INPUT()
	...
	local restquickslot = ui.GetFrame('restquickslot')
	local joystickrestquickslot = ui.GetFrame('joystickrestquickslot')
	local flutingFrame = ui.GetFrame('fluting_keyboard'); -- ?åÏù¥???åÏù¥??- ?ºÎ¶¨ ?∞Ï£º ?µÏä¨Î°?
	local instrumentFrame = ui.GetFrame('instrument_keyboard'); -- ?ÖÍ∏∞ ?•ÎÇúÍ∞??∞Ï£º ?µÏä¨Î°?
